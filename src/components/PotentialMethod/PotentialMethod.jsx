...
const Potential = () => {
  ...
  const validRow = (row) => {
    let flipCount = 0;
    for (let i = 0; i < boardSize; ++i) if (cards[row][i].flipped) flipCount++;
    return flipCount < boardSize / 2;
  };

  const validCol = (col) => {
    let flipCount = 0;
    for (let i = 0; i < boardSize; ++i) if (cards[i][col].flipped) flipCount++;
    return flipCount < boardSize / 2;
  };

  const getNumFaceUp = () => {
    let count = 0;
    for (const row of cards) for (const card of row) if (card.flipped) count++;
    return count;
  };
          ...
          <div className="legend">
            ...
            <div className="stats">
              <div className="stat-row">
                ...
                <p>{boardSize * boardSize - getNumFaceUp()}</p>
              </div>
              <div className="stat-row">
                <div className="card-icon">
                  <Card flipped={true} />
              ...
              <p>
                ratio of face up tiles:{" "}
                {Math.round((getNumFaceUp() / (boardSize * boardSize)) * 100) /
                  100}
              </p>
            </div>
          </div>
